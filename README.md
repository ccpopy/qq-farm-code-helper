...
一个面向 Windows 桌面客户端的小型 Rust/Tauri 应用。它会在本机临时启用定向代理，仅处理目标农场网关：既可以从 `/prod/ws` 登录请求中提取并阻断一次性 `code`，也可以按需透传官方登录，从成功的 `SyncAll` 响应中同步 QQ 游戏好友。
...
1. 先确保当前 QQ 对应的远程账号已经存在且正在运行。
2. 点击「同步官方好友」，确认后重新进入 QQ 农场；这次官方登录会正常完成并消耗一次性 Code。
3. 工具只读取首个成功 `FriendService.SyncAll` 响应中的好友 OpenID，去重后调用远程账号的好友同步接口。
4. 完成后自动恢复系统代理并移除临时证书。好友 OpenID 只在内存和本次请求中存在，不写入本地文件；账号不存在时也不会自动创建。

不使用「同步官方好友」时，Code 获取流程和远程项目原有的好友获取逻辑均不受影响。
...
- Code 不写日志、不写配置文件。
- 好友同步只识别服务端成功的 `FriendService.SyncAll` 响应；好友 OpenID 不写日志、不写配置文件，也不会发送到配置服务器以外的地址。
